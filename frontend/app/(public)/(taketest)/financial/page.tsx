...
import { finantialQuestions } from "@/constants";
...
const Financial = () => {
  ...
  //   await postScore({ financial: totalScore }, user.id);
  // };
  ...
  const handleNext = async () => {
    try {
      ...
      localStorage.setItem("financialScore", totalScore.toString());
      // await postScore({ financial: totalScore }, user.id);
    } catch (error) {
        ...
        <section className="relative flex h-32 items-end bg-gray-900 lg:col-span-5 lg:h-full xl:col-span-4">
          ...
          <div className="hidden lg:relative lg:block lg:p-12">
            ...
            <h2 className="mt-6 text-2xl font-bold text-white sm:text-3xl md:text-4xl">
              Financial Wellness 🤦🏻‍♀️🤦‍♂️
            </h2>

            <p className="mt-4 leading-relaxed text-white/90">
              These are the things I do well ( my daily routines, habits, and
              valued life activities that build and maintain my Financial
              wellness)
            </p>
          </div>
        </section>
